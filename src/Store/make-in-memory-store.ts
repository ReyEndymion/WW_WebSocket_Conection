	...
	return {
		...
		loadMessages: async(jid: string, count: number, cursor: WAMessageCursor) => {
			...
			if(list && mode === 'before' && (!cursorKey || cursorValue)) {
				...
				if(diff < 0) {
					messages = messages.slice(-count) // obtener los últimos X mensajes
				}
			} else {
				messages = []
			}
			...
			return messages
				...
				/**
		 * Obtenga todas las etiquetas disponibles para el perfil
		 *
		 * Tenga en cuenta que la lista se forma a partir de etiquetas y etiquetas predefinidas
		 * que fueron "capturados" durante su edición.
		 */
		getLabels: () => {
			return labels
		},
		...
		/**
		 * Obtener etiquetas para el chat
 		 *
 		 * ID de etiqueta @devoluciones
		 **/
